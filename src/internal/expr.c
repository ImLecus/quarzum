...
Node* parseExpr(PARSING_POS){
    Node* left = parsePrimaryExpr(tokens, i);
    ...
    while (getToken(tokens, *i).value != NULL)
    {
        ...
        ++(*i);
        Node* right = parseExpr(tokens, i);
        Node* expr = createNode(BinaryExpr, 2);
        expr->data = op.value;
        addChildNode(expr, left);
